...
class MainController extends BaseController
{
    ...
    public function agregarPaciente($request, $response, $args){
        ...
        $json = $request->getBody();
        $body = json_decode($json, true);
        if (!(isset($body["id_cita"]) && isset($body['id']) && isset($body['fecha_cita']))){
            return $response->withStatus(400);
        }
        $id_paciente = str_replace(" ", "", $body['id']);
        $id_cita = str_replace(" ", "", $body['id_cita']);
        // FIN Validacion
        $sql = "INSERT INTO acceso_main (`id_cita`, `id_paciente`, `fecha_hora_cita`) VALUES (:id_cita, :id_paciente, :fecha_cita)";
        $statement = $db->prepare($sql);
