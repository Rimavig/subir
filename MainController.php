...
class MainController extends BaseController
{
             ...
             return $response->withStatus(400);
               ...
               if ($item){
                 if ($item->estado=="B" | $item->estado=="I") {
                   ...
                   //$client->sendMail("1", $item->correo, "",($item->nombres).' '.($item->apellidos), "", $six_digit_random_number);
    ...
    public function registro($request, $response, $args){
        ...
        //$client->sendMail("2", $correo, "",($nombre).' '.($apellido), "", "");
    ...
    public function insertRegistro_gratuito($request, $response, $args){
        ...
        try {
          ...
          while($item = $statement->fetch()){
             ...
             'tipo'=> $item->tipo,'sala'=> $item->sala,'precio'=>"",'fecha'=> $item->fecha,'estado'=> $item->estado,'qr'=>$auth_token1,'asientos'=>$asient);
                  ...
                  if($valores!=""){
                    $valor= explode(',',trim($valores));
                      ...
                      try {
                          $id_ticket=0;
                          if ($id_compra!=0) {
                            $sql = "INSERT INTO teatro.tsa_ticket (id_funcion,id_usuario_cliente,id_compra,sala,precio,tipo,usuario_creacion,descuento)
                            VALUES (:id_funcion,:id_usuario_cliente,:id_compra,:sala,:precio,:tipo,:usuario_creacion,:descuento)";
                            $statement = $db->prepare($sql);
                            $statement->bindValue(':id_funcion', $id_funcion, \PDO::PARAM_STR);
                            $statement->bindValue(':id_usuario_cliente', $user_id, \PDO::PARAM_STR);
                            $statement->bindValue(':id_compra', $id_compra, \PDO::PARAM_STR);
    ...
    public function getFuncion_lector($request, $response, $args){
        ...
        try {
            ...
            $horarios[]=array('id_funcion'=>$id_funcion,'fecha'=>$funcion,'aforo'=>$aforo,'cantidad_vendida'=>$cant,'cantidad_ingresada'=>$cant2,'cantidad_retorno'=>$cant3,'tipo'=>$tipo ) ;
        } catch (\PDOException $th) {
            $result = false;
            $out["codigo"] = "102";
            $out["mensaje"] = $error_102_mensaje;
    ...
    public function insert_codigo($request, $response, $args){
        ...
        try {
            ...
            if ($tipo=="1") {
              ...
              $sql ="SELECT te.nombre ,trg.estado ,trg.id_registro_gratuito,tf.id_funcion, ts.nombre as sala, tf.fecha FROM tsa_evento te INNER JOIN tsa_funcion tf ON te.id_evento =tf.id_evento INNER JOIN tsa_sala_mapa tsm ON tsm.id_sala_mapa =te.id_sala_mapa
              ...
              $tipo="";
              while($item = $statement->fetch()){
                    $horarios=array('evento'=>$item->nombre,'sala'=>$item->sala,'fecha'=>$item->fecha, 'id_registro_gratuito'=>$item->id_registro_gratuito,'estado'=>$item->estado) ;
                    if ($id_funcion!=$item->id_funcion) {
                      $out["codigo"] = "228";
    ...
    public function registrar_ingreso_gratuito($request, $response, $args){
        ...
        $json = $request->getBody();
        $body = json_decode($json, true);
        if (!(isset($body["id_registro_gratuito"]))){
          $out["codigo"] = "201";
          $out["mensaje"] = $error_201_mensaje;
          $out["causa"] = $error_201_causa;
          $response->getBody()->write(json_encode($out));
        ...
        try {
            date_default_timezone_set('America/Lima');
            $sql = "UPDATE tsa_registro_gratuito SET estado='I', fecha_ingreso=now()  WHERE id_registro_gratuito=:id_registro_gratuito";
            $statement1 = $db->prepare($sql);
