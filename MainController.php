...
class MainController extends BaseController
{
    ...
    public function insertEsperaS($request, $response, $args){
        ...
        try {
            ...
            foreach($asientos as $llave => $valores) {
              ...
              $id_platea=$items[0];
              $cantidad=$items[1];
              $sql = "SELECT tp.aforo ,tp.vendido as vendido1,tpf.* FROM tsa_platea tp INNER JOIN tsa_platea_funcion tpf ON tp.id_platea = tpf.id_platea and id_funcion=:funcion and tpf.id_platea =:platea";
              $statement = $db->prepare($sql);
              ...
              while($item = $statement->fetch()){
                  ...
                  $aforo=$item->aforo;
                  $cantidad_vendida=$item->vendido1;
                  $cantidad_bloqueado=$item->cantidad_bloqueado;
                  $cantidad_cortesia=$item->cantidad_cortesia;
                  $cantidad_espera=$item->cantidad_espera;
                  $total=$aforo-$cantidad_vendida-$cantidad_bloqueado-$cantidad_cortesia-$cantidad_espera;
    ...
    public function getContacto($request, $response, $args){
        ...
        while($item = $statement->fetch()){
          ...
          'facebook'=> $item->facebook, 'instagram'=> $item->instagram, 'twitter'=> $item->twitter, 'whatsapp'=> $item->whatsapp, 'logo_light'=>$ruta_logo.$item->logo_light, 'logo_dark'=> $ruta_logo.$item->logo_dark);
        }
       if ($band) {
         $out["codigo"] = "208";
    ...
    public function verificarCuenta($request, $response, $args){
        ...
        try {
            ...
            while($item = $statement->fetch()){
              ...
              $statement1->bindValue(':id_usuario',  $item->id_usuario_cliente, \PDO::PARAM_STR);
              $statement1->execute();
              $out="https://www.teatrosanchezaguilar.org";
            }
        } catch (\PDOException $th) {
            $result = false;
            $out["codigo"] = "102";
    ...
    public function getAmigos_teatro($request, $response, $args){
        ...
        try {
            ...
            $usuario = [] ;
            while($item = $statement->fetch()){
                $usuario= array( 'nombres'=> $item->nombres, 'apellidos'=> $item->apellidos, 'codigo_identificador'=> $item->codigo_identificador, 'puntos_acumulados'=> $item->puntos_acumulados, 'fecha_caducidad'=> $item->fecha_caducidad,'amigo_teatro'=> $item->amigo_teatro);
            }
    ...
    public function getTickets($request, $response, $args){
        ...
        try {
            ...
            while($item = $statement->fetch()){
               ...
               'tipo'=> $item->tipo,'sala'=> $item->sala,'precio'=>"",'fecha'=> $item->fecha,'estado'=> $item->estado,'qr'=>"codigo_graficar",'asientos'=>$asitn1 );
    ...
    public function getPromociones($request, $response, $args){
        ...
        try {
            ...
            $promocion=[];
            $ruta= "http://104.198.222.134/imagenes/evento/";
            $sql = "SELECT te.nombre ,te.id_evento, tfc.cantidad_ticket, tfc.id_factor_compra AS id_promo FROM tsa_evento te INNER JOIN tsa_factor_compra tfc ON te.id_evento = tfc.id_evento and te.id_evento=1 and te.estado='A' and tfc.estado='A'";
            $statement = $db->prepare($sql);
            $result = $statement->execute();
            $band=false;
            while($item = $statement->fetch()){
            ...
            $sql = "SELECT te.nombre ,te.id_evento, tfc.cantidad_ticket, tfc.id_factor_pago AS id_promo FROM tsa_evento te INNER JOIN tsa_factor_pago tfc ON te.id_evento = tfc.id_evento and te.id_evento =1 and te.estado='A' and tfc.estado='A'";
            $statement = $db->prepare($sql);
            $result = $statement->execute();
            $band=false;
            while($item = $statement->fetch()){
            ...
            $sql = "SELECT te.nombre ,te.id_evento, tfc.cantidad_ticket, tfc.id_cruzados AS id_promo FROM tsa_evento te INNER JOIN tsa_cruzados tfc ON te.id_evento = tfc.id_evento and te.id_evento !=1 and te.estado='A' and tfc.estado='A'";
            $statement = $db->prepare($sql);
            $result = $statement->execute();
            while($item = $statement->fetch()){
              if (!in_array($item->id_evento, $eventos)) {
        ...
        try {
            ...
            while($item = $statement->fetch()){
              ...
              if ($cantidad_ticket==0) {
                ...
                'id_platea'=> $item->id_platea,'id_funcion'=> $item->id_funcion,'cantidad_ticket'=> $item->cantidad_ticket,'tipo'=>'BT');
              }else{
                ...
                while($item1 = $statement1->fetch()){
                  if ($item1->total < $cantidad_ticket) {
                    $canti=$cantidad_ticket-$item1->total;
                    $promocion[]= array('id_promocion'=> $item->id_banco_tarjeta, 'nombre'=> $item->nombre, 'tarjeta'=> $item->nombreT, 'tipo_tarjeta'=> $item->tipoT, 'descuento'=> $item->descuento, 'descripcion'=> $item->descripcion, 'amigo_teatro'=> $item->amigo_teatro,
                    'id_platea'=> $item->id_platea,'id_funcion'=> $item->id_funcion,'cantidad_ticket'=> $canti."",'tipo'=>'BT');
                  }
                }
              }
    ...
    public function insertRegistro_gratuito($request, $response, $args){
        ...
        if (!(isset($body["nombres"]) && isset($body["apellidos"]) && isset($body["cedula"])  && isset($body["correo"])  && isset($body["celular"])
         && isset($body["fecha_nacimiento"]) && isset($body["id_funcion"])  && isset($body["id_usuario"]))){
          $out["codigo"] = "201";
          $out["mensaje"] = $error_201_mensaje;
          $out["causa"] = $error_201_causa;
          $response->getBody()->write(json_encode($out));
        ...
        $id_usuario= trim($body["id_usuario"]);
        try {
          $sql = "INSERT INTO teatro.tsa_registro_gratuito (nombres,apellidos,cedula,correo,celular,fecha_nacimiento,id_funcion,usuario_creacion,id_usuario_cliente)
          VALUES (:nombres,:apellidos,:cedula,:correo,:celular,:fecha_nacimiento,:id_funcion,:id_usuario,:id_usuario)";
          $statement = $db->prepare($sql);
          $statement->bindValue(':nombres', $nombres, \PDO::PARAM_STR);
          $statement->bindValue(':apellidos', $apellidos, \PDO::PARAM_STR);
          $statement->bindValue(':cedula', $cedula, \PDO::PARAM_STR);
          ...
          $result = $statement->execute();
          while($item = $statement->fetch()){
              $ticket= array('id_ticket'=> "0", 'nombre'=> $item->nombre, 'duracion'=> $item->duracion, 'imagen'=> $ruta_evento.$item->id_evento."H.png",
             'tipo'=> $item->tipo,'sala'=> $item->sala,'precio'=>"",'fecha'=> $item->fecha,'estado'=> $item->estado,'qr'=>"codigo_graficar",'asientos'=>$asient);
    ...
    public function compra_diners($request, $response, $args){
        ...
        try {
            if ($ban) {
              try {
                  $sql = "INSERT INTO teatro.tsa_compra (id_facturacion,id_usuario_cliente,dolares_canjeados,donacion,sub_total,descuento,iva,total,usuario_creacion,id_transacion,authorization_code,status_transacion,tipo)
                  VALUES (:id_facturacion,:id_usuario_cliente,:dolares_canjeados,:donacion,:sub_total,:descuento,:iva,:total,:usuario_creacion,:id_transacion,:authorization_code,:status_transacion,:tipo)";
                  $statement = $db->prepare($sql);
                  $statement->bindValue(':id_facturacion', $id_facturacion, \PDO::PARAM_STR);
    ...
    public function getAlquiler_id($request, $response, $args){
        ...
        try {
            ...
            $sql = "SELECT * FROM tsa_informacion_galeria WHERE id_informacion_tabla=:id_realizados";
            $statement = $db->prepare($sql);
            $statement->bindValue(':id_realizados', $id_realizados, \PDO::PARAM_STR);
            $result = $statement->execute();
