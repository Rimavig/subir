                  ...
                  if ($result) {
                    ...
                    if ($band) {
                        try {
                            ...
                            if ($band1) {
                                $out["mensaje"] = array('mensaje'=>"Código valido",'status'=>"A",'tipo'=>$residente,'celular'=>$celular,'Residente'=> $usuario, 'Estudiante'=>"");
                            }else{
        ...
        try {
          if ($tipo=="R") {
            ...
            while($item = $statement->fetch()){
                $band=false;
                $band1=true;
                if (!in_array($item->celular, $padres)) {
                  ...
                  if ($band1==false) {
                    ...
                    $registro[]=array('padre'=>$item->padre,'celular'=>$item->celular,'id_aula'=>$item->id_aula,'hijos'=>"",'observacion'=>$item->observacion,'fecha'=>$item->fecha);;
                  }
                    $padres[]=$item->celular;
                }
        ...
        try {
          foreach($historial as $llave => $valores) {
            ...
            if (isset($historial1[1])) {
              ...
              $statement->bindValue(':celular',  $celular, \PDO::PARAM_STR);
              $statement->bindValue(':tipo',  $tipo, \PDO::PARAM_STR);
              $statement->bindValue(':hijo',  $historial1[1], \PDO::PARAM_STR);
