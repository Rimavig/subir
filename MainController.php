...
class MainController extends BaseController
{
    ...
    public function updateFacturacion($request, $response, $args){
        ...
        $json = $request->getBody();
        $body = json_decode($json, true);
        if (!(isset($body["nombres"])  && isset($body["id_facturacion"])  && isset($body["apellidos"]) && isset($body["cedula"]) && isset($body["razon_social"]) && isset($body["ruc"]) && isset($body["tipo"]))){
            $out["codigo"] = "201";
            $out["mensaje"] = $error_201_mensaje;
            $out["causa"] =  $error_201_causa;
            $response->getBody()->write(json_encode($out));
        ...
        $tipo = trim($body['tipo']);
        try {
            $sql = "UPDATE teatro.tsa_facturacion SET nombres=:nombres, apellidos=:apellidos, razon_social=:razon_social, cedula=:cedula, ruc=:ruc, tipo=:tipo WHERE id_facturacion=:id_facturacion";
            $statement = $db->prepare($sql);
    ...
    public function getPromociones($request, $response, $args){
        ...
        try {
            ...
            $promocion=[];
            $ruta= "http://104.198.222.134/imagenes/evento/";
            $sql = "SELECT te.nombre ,te.id_evento, tfc.cantidad_ticket, tfc.id_factor_compra AS id_promo FROM tsa_evento te INNER JOIN tsa_factor_compra tfc ON te.id_evento = tfc.id_evento ";
            $statement = $db->prepare($sql);
            $result = $statement->execute();
            while($item = $statement->fetch()){
              if (!in_array($item->id_evento, $eventos)) {
            ...
            $sql = "SELECT te.nombre ,te.id_evento, tfc.cantidad_ticket, tfc.id_cruzados AS id_promo FROM tsa_evento te INNER JOIN tsa_cruzados tfc ON te.id_evento = tfc.id_evento ";
            $statement = $db->prepare($sql);
            $result = $statement->execute();
            while($item = $statement->fetch()){
              if (!in_array($item->id_evento, $eventos)) {
            ...
            $sql = "SELECT te.nombre ,te.id_evento, tfc.cantidad_ticket, tfc.id_factor_pago AS id_promo FROM tsa_evento te INNER JOIN tsa_factor_pago tfc ON te.id_evento = tfc.id_evento ";
            $statement = $db->prepare($sql);
            $result = $statement->execute();
            while($item = $statement->fetch()){
              if (!in_array($item->id_evento, $eventos)) {
...
//
    ...
    public function insertRegistro_gratuito($request, $response, $args){
        ...
        $id_usuario= trim($body["id_usuario"]);
        try {
          $sql = "INSERT INTO teatro.tsa_registro_gratuito (nombres,apellidos,cedula,correo,celular,fecha_nacimiento,id_funcion,usuario_creacion)
          VALUES (:nombres,:apellidos,:cedula,:correo,:celular,:fecha_nacimiento,:id_funcion,:id_usuario)";
          $statement = $db->prepare($sql);
          $statement->bindValue(':nombres', $nombres, \PDO::PARAM_STR);
          $statement->bindValue(':apellidos', $apellidos, \PDO::PARAM_STR);
          $statement->bindValue(':cedula', $cedula, \PDO::PARAM_STR);
