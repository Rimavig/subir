...
class MainController extends BaseController
{
    ...
    public function getTickets($request, $response, $args){
        ...
        try {
            ...
            $result = $statement->execute();
            while($item = $statement->fetch()){
                $ticketT[]= array('id_ticket'=> $item->id_registro_gratuito, 'nombre'=> $item->nombre, 'duracion'=> $item->duracion, 'imagen'=> $ruta_evento.$item->id_evento."V.png",
    ...
    public function getFuncion($request, $response, $args){
        ...
        try {
            ...
            $funcion= [];
            while($item = $statement->fetch()){
              $funcion []= array('id_funcion'=> $item->id_funcion,'platea'=> $platea,'asientos'=> $asientosT, 'aforo_funcion'=> $item->aforo, 'vendido_funcion'=> $item->cantidad_vendida, 'tipo'=> $tipo ,'imagen'=> $ruta_distribucion.$id_evento.".png");
            }
        } catch (\PDOException $th) {
            $result = false;
            $out["codigo"] = "102";
    ...
    public function insertRegistro_gratuito($request, $response, $args){
        ...
        try {
          ...
          $result = $statement->execute();
          while($item = $statement->fetch()){
              $ticket= array('id_ticket'=> "0", 'nombre'=> $item->nombre, 'duracion'=> $item->duracion, 'imagen'=> $ruta_evento.$item->id_evento."V.png",
