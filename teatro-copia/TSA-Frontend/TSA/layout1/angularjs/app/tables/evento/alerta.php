...
//MANTENIMIENTO
    ...
    //EDITAR
        ...
        $id=$_POST["id_evento"];
        $re = $client->updateEvento_informacion($nombre,$duracion,$fechaI,$fechaf,$productora,$salamapa,$tipoE,$espectaculo,$categoria,$clasificacion,$procedencia,$aforo,"V","A",$id, $_SESSION["usuario"]);
        $resultado = "".$re;
        $band2=true;
    } else if ($_POST["tipo"]==="Egratuito") {
        $id=$_POST["id_evento"];
        $re = $client->updateEvento_informacion($nombre,$duracion,$fechaI,$fechaf,$productora,$salamapa,$tipoE,$espectaculo,$categoria,$clasificacion,$procedencia,$aforo,"G","A",$id, $_SESSION["usuario"]);
        $resultado = "".$re;
        $band2=true;   
    } else if ($_POST["tipo"]==="Einformativo") {
        $id=$_POST["id_evento"];
        $re = $client->updateEvento_informacion($nombre,$duracion,$fechaI,$fechaf,$productora,$salamapa,$tipoE,$espectaculo,$categoria,$clasificacion,$procedencia,$aforo,"I","A",$id, $_SESSION["usuario"]);
        $resultado = "".$re;
        $band2=true;  
    } else if ($_POST["tipo"]==="Esinopsis") {
        $id=$_POST["id_evento"];
        $sinopsis=$_POST["sinopsis"];
        $re = $client->updateEvento_sinopsis($sinopsis,"V",$id,$_SESSION["usuario"]);
        $resultado = "".$re;
        $band2=true;
    //ESTAD0
    ...
    if ($band2) {
        if ($_POST["tipo2"]==="crear") {
            if($resultado=="true"){
                if ($_POST["tipo"]==="venta" | $_POST["tipo"]==="gratuito" | $_POST["tipo"]==="informativo"){
                    ...
                    ?>
                    <script type="text/javascript"> 
                        alert("Se creo la <?php echo $_POST["nombreT"];?> correctamente");
            ...
            if($resultado=="true"){
                if ($_POST["tipo"]==="Eventa"){
                    ...
                    ?>
                    <script type="text/javascript"> 
                         alert("Se Edito el video correctamente");
                        $('.page-spinner-loader').removeClass('hide');
        ...
        if($_POST["tipo"]==="imagenC" | $_POST["tipo"]==="imagenV" | $_POST["tipo"]==="imagenH"){
            ?>
            <script type="text/javascript"> 
                alert("Se edito correctamente la imagen");
                $('.page-spinner-loader').removeClass('hide');
                $('#multimedia').load('./tables/evento/tab_multimedia.php', {var1:<?php echo $_POST["id_evento"];?>,tipo:"venta"},function() { 
                    $('.page-spinner-loader').addClass('hide');
