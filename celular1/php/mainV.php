...
if (isset($_POST["ciudadela"]) ) {
    ...
    $dato = explode(',',$resultado);
      ...
      if ($cont>6) {
        ...
        if (isset($dato[3])) {$_SESSION["correo"]= $dato[3]; }
        if (isset($dato[4])) {
          $_SESSION["status"]= $dato[4];
          if ($dato[4]!="A") {
            $resultado1="disable";
          }
        }
        if (isset($dato[5])) {
          $_SESSION["invitacion"]= $dato[5];
        }
        if (isset($dato[6])) {$_SESSION["ciudadelas"]= $dato[6]; }
    }else{
      if (isset($dato[0])) {$_SESSION["id"]= $dato[0]; }
      if (isset($dato[1])) {$_SESSION["nombres"]= $dato[1]; }
      if (isset($dato[2])) {$_SESSION["apellidos"]= $dato[2]; }
...
<header class="menu_visitante">
  ...
  <nav class="nav_bar">
    <ul  >
      ...
      <li><a href="#" name="link1" id="link1" onclick="limpiar()"><span class="icon-exit"></span>Cerrar Sesión</a></li>
    </ul>
  </nav>
  <nav class="nav2_bar">
    <ul class=" menu_P">
