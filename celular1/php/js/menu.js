...
function compartir1(tipo) {
	console.log(tipo);
	$('.container-fluid').load("https://api.whatsapp.com/send?text=" + encodeURIComponent(tipo));
...
function enviarR() {
	...
	if( inicio < termino ) {
	  ...
	  tiempo="mal";
		...
		$(".container1").hide();
		$('.invitacion-enviar').load('enviarR.php',{nombres:nombres,apellidos:apellidos,email:email,celular:celular,inicio:inicio,termino:termino,tiempo:tiempo});
		$(".invitacion-enviar").show();
}
...
function registrarse() {
	$('html, body').animate({scrollTop:0}, 'slow');
	 ...
	 $(".cont2").hide();
	 $(".cont1").show();
	 $('.cont1').load('tipoR.php', {tipo:tipo,nombres:nombres,
		 apellidos:apellidos,email:email,contraseña:contraseña,
		 contraseña1:contraseña1,celular:celular,ciudadela:ciudadela
	 });
}
