    ...
    public function createCorp($request, $response, $args){
          ...
          if ($item==false) {
            ...
            if ($item){
              if ($item->estado=="B" | $item->estado=="I") {
                ...
                $client->sendMail1("1", $item->correo, "",($item->nombres), $six_digit_random_number);
          ...
          if ($item==false) {
            ...
            if ($item){
              if ($item->estado=="B" | $item->estado=="I") {
                ...
                $statement1->bindValue(':id_usuario',  $item->id_usuario_cliente, \PDO::PARAM_STR);
                $result = $statement1->execute();
                $client->sendMail1("8", $item->correo, "",($item->nombres), "");
      ...
      public function registro($request, $response, $args){
        ...
        try {
          ...
          $client->sendMail1("2", $correo, "",($nombre), $id_usuario);
        } catch (\PDOException $th) {
          $result = false;
          $errocode = $th->getCode();
      ...
      public function getEventos($request, $response, $args){
        ...
        try {
          ...
          $sql = "SELECT te.*, tc.nombre as categoria,tf.estado as estadoF, tf.fecha, tf.id_funcion FROM tsa_evento te  INNER JOIN  tsa_categoria tc ON te.id_categoria=tc.id_categoria and te.estado='A' INNER JOIN  tsa_funcion tf ON tf.id_evento =te.id_evento  ORDER BY tf.fecha ASC ";
          $statement = $db->prepare($sql);
          $result = $statement->execute();
                  ...
                  if ($item->estadoF=="A") {
                    ...
                    if (!$band) {
                      if (in_array($item->id_evento, $estrella[0])) {
                        ...
                        $estrellaT[]= array('id'=> $item->id_evento, 'nombre'=> $item->nombre, 'duracion'=> $item->duracion, 'imagen'=> $ruta_evento.$item->id_evento."H.png", 'tipo'=> $item->tipo, 'categoria'=> $categoria, 'sinopsis'=> $item->sinopsis, 'preventa'=> $item->preventa);
                      }
                    }
                    if ($band2) {
                      $band3=false;
                      $cartelera[]= array('id'=> $item->id_evento, 'nombre'=> $item->nombre, 'duracion'=> $item->duracion, 'imagen'=> $ruta_evento.$item->id_evento."C.png", 'tipo'=> $item->tipo, 'categoria'=> $categoria, 'sinopsis'=> $item->sinopsis, 'preventa'=> $item->preventa);
                    }
          ...
          foreach ($eventosI as $clave) {
            if (!in_array($clave, $eventos)) {
              $sql1 = "UPDATE tsa_evento	SET estado='I' WHERE id_evento=:id_evento";
              $statement1 = $db->prepare($sql1);
              $statement1->bindValue(':id_evento',  $clave, \PDO::PARAM_STR);
              $statement1->execute();
            }
          }
      ...
      public function getTickets($request, $response, $args){
        ...
        try {
          ...
          while($item = $statement->fetch()){
            ...
            $ticketT[]= array('id_ticket'=> $item->id_registro_gratuito,'id_evento'=> $item->id_evento,  'nombre'=> $item->nombre, 'duracion'=> $item->duracion, 'imagen'=> $ruta_evento.$item->id_evento."H.png",
      ...
      public function getFuncion($request, $response, $args){
        ...
        try {
          ...
          if ($bandw) {
            ...
            while($item = $statement->fetch()){
              $bandw= false;
              $platea []= array('id_platea'=> $item->id_platea, 'nombre'=> $item->nombre, 'precio'=> $item->costo, 'aforo_platea'=> $item->aforo,'vendido_platea'=> "0");
            }
          }
          $acm=1;
          $platea1=[];
        ...
        try {
          ...
          while($item = $statement->fetch()){
            ...
            if ($cantidad_ticket==0) {
              ...
              $sql1= "SELECT COUNT(id_promocion) as total FROM tsa_ticket_promocion_codigo WHERE id_promocion =:id_promo and id_usuario_cliente =:id_usuario and fecha_creacion < DATE_SUB(NOW() , INTERVAL 11 MONTH)";
              $statement1 = $db->prepare($sql1);
              ...
              while($item1 = $statement1->fetch()){
                if ($item1->total < $cantidad_ticket) {
                  ...
                  $result = false;
                  $out["codigo"] = "233";
                  $out["mensaje"] = $error_233_mensaje;
                  $out["causa"] = $error_233_causa;
                  $response->getBody()->write(json_encode($out));
                  return $response->withStatus(500);
                }
              }
      ...
      public function insertRegistro_gratuito($request, $response, $args){
        ...
        try {
          ...
          while($item = $statement->fetch()){
            ...
            $client->sendMail1("9", $correo, $id_ticket,($nombres),$auth_token1);
      ...
      public function verificacion_diners($request, $response, $args){
        ...
        try {
          ...
          $order["description"]="Teatro Sanchez Aguilar-Compra de Ticket App ".$tipoA;
                    ...
                    if ($id_promocion!=0) {
                      if ($tipo_promocion=="FC" | $tipo_promocion=="CD" | $tipo_promocion=="FP" | $tipo_promocion=="BT" | $tipo_promocion=="CP") {
                        if ($tipo_promocion=="FC") {
                          ...
                          $statement->execute();
                          if ($id_promocion==10) {
                            $sql = "UPDATE tsa_codigo_promocional_especial	SET estado='I' WHERE id_usuario_cliente=:id_usuario_cliente and tipo='R' and fecha_creacion >=DATE_SUB(NOW() , INTERVAL 30 day)";
                            $statement = $db->prepare($sql);
                            $statement->bindValue(':id_usuario_cliente', $user_id, \PDO::PARAM_STR);
                            $statement->execute();
                          }
                          if ($id_promocion==11) {
                            $sql = "UPDATE tsa_codigo_promocional_especial	SET estado='I' WHERE id_usuario_cliente=:id_usuario_cliente and tipo='C' and fecha_creacion >=DATE_SUB(NOW() , INTERVAL 30 day)";
                            $statement = $db->prepare($sql);
                            $statement->bindValue(':id_usuario_cliente', $user_id, \PDO::PARAM_STR);
                            $statement->execute();
                          }
                    ...
                    if ($id_promocion!=0) {
                      if ($tipo_promocion=="FC" | $tipo_promocion=="CD" | $tipo_promocion=="FP" | $tipo_promocion=="BT" | $tipo_promocion=="CP") {
                        if ($tipo_promocion=="FC") {
                          ...
                          $statement->execute();
                          if ($id_promocion==10) {
                            $sql = "UPDATE tsa_codigo_promocional_especial	SET estado='I' , fecha_modificacion=now() WHERE id_usuario_cliente=:id_usuario_cliente and tipo='R' and fecha_creacion >=DATE_SUB(NOW() , INTERVAL 30 day)";
                            $statement = $db->prepare($sql);
                            $statement->bindValue(':id_usuario_cliente', $user_id, \PDO::PARAM_STR);
                            $statement->execute();
                          }
                          if ($id_promocion==11) {
                            $sql = "UPDATE tsa_codigo_promocional_especial	SET estado='I', fecha_modificacion=now() WHERE id_usuario_cliente=:id_usuario_cliente and tipo='C' and fecha_creacion >=DATE_SUB(NOW() , INTERVAL 30 day)";
                            $statement = $db->prepare($sql);
                            $statement->bindValue(':id_usuario_cliente', $user_id, \PDO::PARAM_STR);
                            $statement->execute();
                          }
      ...
      public function compra($request, $response, $args){
        ...
        try {
          if ($ban) {
            ...
            $result = false;
            $out["codigo"] = "229";
            $out["mensaje"] = $error_229_mensaje;
            $out["causa"] = $error_229_causa;
            $response->getBody()->write(json_encode($out));
            return $response->withStatus(400);
          }
        } catch (\PDOException $th) {
      ...
      public function getTeatro($request, $response, $args){
        ...
        try {
          ...
          $sql = "SELECT * FROM tsa_evento WHERE estado!='A' and estado!='P' ORDER BY  CAST(  id_evento AS DECIMAL) ASC";
          //$sql = "SELECT * FROM categorias where  id_tienda =:tienda";
          $statement = $db->prepare($sql);
          $result = $statement->execute();
          $ruta= "https://teatrosanchezaguilar.org/imagenes/evento/";
      ...
      public function enviarContactanos($request, $response, $args){
        ...
        try {
          ...
          $client->sendMail1("10", "teatrosanchezaguilar@gmail.com", "","", $id_envio);
        } catch (\PDOException $th) {
          $result = false;
          $out["codigo"] = "102";
          $out["mensaje"] = $error_102_mensaje;
      ...
      public function enviarContactanosAlquiler($request, $response, $args){
        ...
        try {
          ...
          $result = $statement->execute();
          $id_envio=$db->lastInsertId();
          $client->sendMail1("11", "teatrosanchezaguilar@gmail.com", "","", $id_envio);
        } catch (\PDOException $th) {
          $result = false;
          $out["codigo"] = "102";
          $out["mensaje"] = $error_102_mensaje;
      ...
      public function getEvento_lector($request, $response, $args){
        ...
        try {
          ...
          $datetime = date("Y-m-d H:i:s");
          $band5=false;
          $sql = "SELECT te.id_evento,te.duracion ,  tf.fecha, tf.aforo , tf.cantidad_vendida, tf.estado as estado_funcion, tf.id_funcion FROM tsa_evento te INNER JOIN  tsa_funcion tf ON tf.id_evento =te.id_evento  and te.id_evento =:id_evento ORDER BY tf.fecha ASC";
          $statement = $db->prepare($sql);
          $statement->bindValue(':id_evento', $id_evento, \PDO::PARAM_STR);
          $result = $statement->execute();
          $datetime = date("Y-m-d H:i:s");
