...
class MainController extends BaseController
{
          ...
          return $response;
            ...
            if ($item){
                if ($item->contrasena == $password) {
                  $out['usuario'][] = array('id'=> $item->id_usuario_cliente, 'nombres'=> $item->nombres,'apellidos'=> $item->apellidos, 'usuario'=> $item->usuario, 'cedula'=> $item->cedula, 'correo'=> $item->correo, 'sexo'=> $item->sexo, 'celular'=> $item->celular,
           ...
           if ($item==false) {
             ...
             return $response;
               ...
               if ($item){
                  ...
                  $out["status"] = "Se reseteo contraseña correctamente";
                   $response->getBody()->write(json_encode($out));
                   return $response;
               }
           }
               ...
               if ($item){
                 ...
                 if ($item->codigo == $codigo) {
                   ...
                   if ($total<10) {
                       ...
                       $statement1->bindValue(':id_usuario', $item->id_usuario_cliente, \PDO::PARAM_STR);
                       $result = $statement1->execute();
                       $out["status"] = "Se cambio contraseña correctamente";
                       $response->getBody()->write(json_encode($out));
                       return $response;
                    }else{
                       $out["status"] = "Caduco codigo";
                    }
                 }else{
                   $out["status"] = "Codigo Incorrecto";
                 }
    ...
    public function registro($request, $response, $args){
        ...
        try {
            ...
            $response = $response->withHeader('Content-Type', 'application/json');
            if ($errocode=="23000") {
                $out['status'][] ="El registro ya existe
        ...
        $response = $response->withHeader('Content-Type', 'application/json');
        $out['status'][] ="Se registro correctamente";
    ...
    public function getCategorias($request, $response, $args){
        ...
        while($item = $statement->fetch()){
          $band=false;
          $out['categorias'][] = array('id'=> $item->id_categoria, 'nombre'=> $item->nombre);
        }


       if ($band) {
         $out["status"] = "No existe categorias activas";
       }else{
           $response->getBody()->write(json_encode($out));
       }
    ...
    public function getContacto($request, $response, $args){
        ...
        while($item = $statement->fetch()){
          $band=false;
          $out['contacto'][] = array('id'=> $item->id_contacto, 'nombre'=> $item->nombre, 'celular'=> $item->celular, 'telefono'=> $item->telefono, 'direccion'=> $item->direccion, 'correo'=> $item->correo, 'pagina'=> $item->pagina,
          'facebook'=> $item->facebook, 'instagram'=> $item->instagram, 'twitter'=> $item->twitter, 'logo_light'=> $item->logo_light, 'logo_dark'=> $item->logo_dark);
        }


       if ($band) {
         $out["status"] = "No existe contacto";
       }else{
           $response->getBody()->write(json_encode($out));
       }
    ...
    public function getEventos($request, $response, $args){
        ...
        try {
            ...
            $sql = "SELECT * FROM tsa_evento_destacado";
            $statement = $db->prepare($sql);
            $result = $statement->execute();
            while($item = $statement->fetch()){
              $band1=false;
              if (!in_array($item->id_evento, $estrella[0])) {
                  $destacados[]=$item->id_evento;
              }
            }
            if ($band1) {
                $destacados[]=array();
              ...
              if ($item->fecha<$datetime) {
                ...
                if (!in_array($item->id_evento, $eventos)) {
                  ...
                  if (!$band) {
                    if (in_array($item->id_evento, $estrella[0])) {
                      $band2=false;
                      $estrellaT[]= array('id'=> $item->id_evento, 'nombre'=> $item->nombre, 'duracion'=> $item->duracion, 'imagen'=> $ruta."\\images\\eventos\\".$item->ruta_imagen, 'tipo'=> $item->tipo, 'categoria'=> $item->categoria);
                    }
                  }
                  if (!$band1) {
                    if (in_array($item->id_evento, $destacados)) {
                      $band2=false;
                      $destacadosT[]= array('id'=> $item->id_evento, 'nombre'=> $item->nombre, 'duracion'=> $item->duracion, 'imagen'=> $ruta."\\images\\eventos\\".$item->ruta_imagen, 'tipo'=> $item->tipo, 'categoria'=> $item->categoria);
                    }
                  }
                  if ($band2) {
                    $band3=false;
                    $cartelera[]= array('id'=> $item->id_evento, 'nombre'=> $item->nombre, 'duracion'=> $item->duracion, 'imagen'=> $ruta."\\images\\eventos\\".$item->ruta_imagen, 'tipo'=> $item->tipo, 'categoria'=> $item->categoria);
                  }
        ...
        $response = $response->withHeader('Content-Type', 'application/json');
        $out['eventos'][] = array('estrella'=> $estrellaT, 'destacados'=> $destacadosT, 'cartelera'=>  $cartelera);
    ...
    public function getEventos_fecha($request, $response, $args){
        ...
        try {
            ...
            while($item = $statement->fetch()){
              ...
              if (!in_array($item->id_evento, $eventos)) {
                  $cartelera[$item->id_evento]= array('id'=> $item->id_evento, 'nombre'=> $item->nombre, 'duracion'=> $item->duracion, 'imagen'=> $ruta."\\images\\eventos\\".$item->ruta_imagen,
                   'tipo'=> $item->tipo, 'categoria'=> $item->categoria,'sala'=> $item->sala,'clasificacion'=> $item->clasificacion);
                  $eventos[]= $item->id_evento;
              }
    ...
    public function getEventos_categoria($request, $response, $args){
        ...
        try {
            ...
            INNER JOIN  tsa_funcion tf ON tf.id_evento =te.id_evento ORDER BY tf.fecha ASC ";
            $statement = $db->prepare($sql);
    ...
    public function getEventos_id($request, $response, $args){
        ...
        try {
            ...
            tte2.nombre as tipo_evento, tf.fecha, tf.estado as estado_funcion FROM tsa_evento te
            INNER JOIN  tsa_categoria tc ON te.id_categoria=tc.id_categoria
            INNER JOIN  tsa_clasificacion tc2 ON tc2.id_clasificacion =te.id_clasificacion
            ...
            INNER JOIN  tsa_tipo_evento tte2 ON tte2.id_tipo_evento=te.id_tipo_evento
            INNER JOIN  tsa_funcion tf ON tf.id_evento =te.id_evento and te.id_evento =:id_evento ORDER BY tf.fecha ASC ";
            $statement = $db->prepare($sql);
            ...
            while($item = $statement->fetch()){
              if (!in_array($item->id_evento, $eventos)) {
                  $cartelera[$item->id_evento]= array('id'=> $item->id_evento, 'nombre'=> $item->nombre, 'duracion'=> $item->duracion, 'imagen'=> $ruta."\\images\\eventos\\".$item->ruta_imagen,
                   'tipo'=> $item->tipo, 'categoria'=> $item->categoria, 'clasificacion'=> $item->clasificacion, 'procedencia'=> $item->procedencia, 'productora'=> $item->productora1,'sala'=> $item->sala,'tipo_espectaculo'=> $item->tipo_espectaculo,
                   'sinopsis'=> $item->sinopsis, 'elenco'=> $item->elenco, 'video'=> $item->ruta_video);
                  $eventos[]= $item->id_evento;
              }
              $horarios[$item->id_evento][]=array('fecha'=>$item->fecha,'estado'=>$item->estado_funcion) ;
            }
            foreach ($eventos as $clave) {
                $cartelera[$clave]['horarios']=$horarios[$clave];
            }
    ...
    public function getTickets($request, $response, $args){
        ...
        try {
            ...
            INNER JOIN  tsa_evento te ON te.id_evento =tf.id_evento and tuc.id_usuario_cliente =:id_usuario";
            $statement = $db->prepare($sql);
            ...
            while($item = $statement->fetch()){
              if (!in_array($item->id_ticket, $tickets)) {
                  $ticket[$item->id_ticket]= array('id_ticket'=> $item->id_ticket, 'nombre'=> $item->nombre, 'duracion'=> $item->duracion, 'imagen'=> $ruta."\\images\\eventos\\".$item->ruta_imagen,
