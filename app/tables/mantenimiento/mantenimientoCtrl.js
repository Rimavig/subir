  ...
  .controller('mantenimientoCtrl', ['$scope','pluginsService', function ($scope ,pluginsService) {
    ...
    //PERFIL CATEGORIA   
        ...
        $(document).on('click', '.editar', function (e) {
            ...
            $('#Ecategoria').load('./tables/mantenimiento/editar_mantenimiento.php', {var1:estado.innerHTML,tipo:tipo},function() {    
                $('.page-spinner-loader').addClass('hide');
                $('#Ecategoria').modal('show'); // abrir
            });
    ...
    //MAPA
        ...
        $(document).on('click', '.editarMS', function (e) {
            e.preventDefault();
            $(this).prop("disabled",true); 
            var estado=$(this).parents('tr').find('#fila0')[0].innerHTML;
            var id=$(this).parents('tr').find('#idMapa')[0].innerHTML;
            var tipo=$(this).parents().find('#tipo')[0].value;
            var nombre=$(this).parents('tr').find('#nombreSala')[0].innerHTML;
            console.log(tipo);
            $('.page-spinner-loader').removeClass('hide');
            $('#EcrearMapaS').load('./tables/mantenimiento/mapa_editar.php', {var1:estado,id:id, nombre:nombre, tipo:tipo},function() {    
                $('.page-spinner-loader').addClass('hide');
                $('#EcrearMapaS').modal('show'); // abrir
            });
            $(this).prop("disabled",false);
        });
    ...
    //IMAGENES    
        $(document).on('change', '#sala', function (e) {
            ...
            image.src = "../../../assets/global/images/sala/"+mapa+".png";
        });
        $(document).on('change', '#imagen_sala', function (e) {
            e.preventDefault();
            var mapa = $(this).val();
            var image = document.getElementById('imagen');
            image.src = "/teatro-copia/imagenes/sala/"+mapa;
        });
        $(document).on('change', '#imagen_distribucion', function (e) {
            e.preventDefault();
            var mapa = $(this).val();
            var image = document.getElementById('imagen');
            image.src = "/teatro-copia/imagenes/distribucion/"+mapa;
        });
