...
if (isset($_POST["tipo"])) {
    ...
    if ( $tipo=="abrirCaja"){
        ...
        $id=$_POST["id"];
        $re = $client->Facturacion($id, "NCA","", $_SESSION["usuario"]);
        $resultado = "".$re;
    }else if ( $tipo=="notaCredito"){
        $id=$_POST["id"];
        $motivo=$_POST["motivo"];
        $re = $client->Facturacion($id, "NC",$motivo, $_SESSION["usuario"]);
        $resultado = "".$re;
    }else if ( $tipo=="notaCreditoParcial"){
        $id=$_POST["id"];
        $motivo=$_POST["motivo"];
        $comando=$_POST["comando"];
        $re = $client->updateGeneral("NCP",$id, $motivo,$comando,  $_SESSION["usuario"]);
        $resultado = "".$re;
