...
angular.module('newApp')
  .controller('facturacionCtrl', ['$scope','pluginsService', function ($scope,pluginsService,) {
    ...
    $scope.$on('$viewContentLoaded', function () {
        ...
        $(document).on('click', '.updateNotaCredito', function (e) {
            e.preventDefault();
            e.stopImmediatePropagation();
            $(this).prop("disabled",true); 
            var id=$(this).parents('tr')[0].children[1].innerHTML;
            if (confirm("Estás seguro de Actualizar Nota Crédito?") == false) {
                $(this).prop("disabled",false);
                return;
            }else{
                $('#alerta').load('./tables/facturacion/alerta.php', {tipo:"updateNotaCredito", id:id},function() {    
                    $('.page-spinner-loader').addClass('hide');
                });
            }
        ...
        $(document).on('click', '.facturarA', function (e) {
            e.preventDefault();
            e.stopImmediatePropagation();
            $(this).prop("disabled",true); 
            var id=$(this).parents('tr').find('.hide_column')[0].innerHTML;
        ...
        $(document).on('click', '.buscarRP', function (e) {
            ...
            var table = $('#table-ventas').DataTable();
            var data="";
            if ($('#table-ventas').hasClass('reportesV_data')) {
                data="tables/facturacion/reportes_venta_data.php?var1="+fechaI+"&var2="+fechaF ;
            }
            if ($('#table-ventas').hasClass('reportesE_data')) {
                data="tables/facturacion/reportes_eliminadas_data.php?var1="+fechaI+"&var2="+fechaF ;
            }
            if ($('#table-ventas').hasClass('reportesN_data')) {
                data="tables/facturacion/reportes_notas_data.php?var1="+fechaI+"&var2="+fechaF ;
            }
        ...
        //PENDIENTE
        $(document).on('click', '.notaCreditoMotivo', function (e) {
            e.preventDefault();
            e.stopImmediatePropagation();
            $(this).prop("disabled",true); 
            var id=$(this).parents().find('#idCompra')[0].value;
            $('#Cusuarios').load('./tables/facturacion/motivo_nota.php', {idCompra:id},function() {    
                $('.page-spinner-loader').addClass('hide');
                $('#Cusuarios').modal('show'); // abrir
                $('.notaCreditoMotivo').prop("disabled",false); 
            });
        });
        //PENDIENTE
        $(document).on('click', '.notaCreditoParcialMotivo', function (e) {
            e.preventDefault();
            e.stopImmediatePropagation();
            $(this).prop("disabled",true); 
            var id=$(this).parents().find('#idCompra')[0].value;
            $('#Cusuarios').load('./tables/facturacion/motivo_nota_parcial.php', {idCompra:id},function() {    
                $('.page-spinner-loader').addClass('hide');
                $('#Cusuarios').modal('show'); // abrir
                $('.notaCreditoParcialMotivo').prop("disabled",false); 
                setTimeout(function(){
                    handleiCheck();
                },200);
            });
        ...
        $(document).on('click', '.notaCreditoParcial', function (e) {
            e.preventDefault();
            e.stopImmediatePropagation();
            $(this).prop("disabled",true); 
            var idCompra=$(this).parents().find('#idCompra')[0].value;
            var motivo=$(this).parents().find('#motivo')[0].value;
            var cantidad=$(this).parents().find('#catidadTickets')[0].value;
            var band=true;
            var comando="";
            var c=0;
            for (var i=1, iLen=cantidad; i<iLen; i=i+1) {
            ...
            if(comando==""){
                var n = noty({
                    text        : '<div class="alert alert-warning "><p><strong>Seleccione tickets</p></div>',
                    layout      : 'topCenter', //or left, right, bottom-right...
                    theme       : 'made',
                    type        : 'error',
                    maxVisible  : 5,
            ...
            if (motivo.length<2) {
                var n = noty({
                    text        : '<div class="alert alert-warning "><p><strong>Ingrese Motivo</p></div>',
                    layout      : 'topCenter', //or left, right, bottom-right...
                    theme       : 'made',
                    type        : 'error',
                    maxVisible  : 5,
            ...
            if (band){
                if (confirm("Estás seguro de crear Nota Crédito?") == false) {
                    ...
                    $('#alerta').load('./tables/facturacion/alerta.php', {tipo:"notaCreditoParcial", id:idCompra,motivo:motivo, comando:comando },function() {    
                        $('.page-spinner-loader').addClass('hide');
                    });
                }
            }else{
        ...
        $(document).on('click', '.notaCredito', function (e) {
            ...
            if (band){
                if (confirm("Estás seguro de crear Nota Crédito?") == false) {
                    ...
                    $('#alerta').load('./tables/facturacion/alerta.php', {tipo:"notaCredito", id:id,motivo:motivo},function() {    
                        $('.page-spinner-loader').addClass('hide');
                    });
                }
            }else{
      ...
      $scope.$on('$destroy', function () {
        ...
        $(document).off('click','.facturarA');
        $(document).off('click','.updateNotaCredito');
        $(document).off('click','.notaCredito');
        $(document).off('click','.notaCreditoMotivo');
